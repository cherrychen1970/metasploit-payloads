...
static json_object_to_json_string_fn json_object_binary_to_json_string;
...
{
	return (jso->o.c_string.len < LEN_DIRECT_STRING_DATA) ? jso->o.c_string.str.data : jso->o.c_string.str.ptr;
}
							  ...
							  const unsigned opts)
{
	...
	existing_entry = (opts & JSON_C_OBJECT_ADD_KEY_IS_NEW) ? NULL : lh_table_lookup_entry_w_hash(jso->o.c_object, (const void *)key, hash);
	...
	if (!existing_entry)
	{
		const void *const k = (opts & JSON_C_OBJECT_KEY_IS_CONSTANT) ? (const void *)key : _strdup(key);
		if (k == NULL)
			return -1;
...
struct json_object *json_object_new_string(const char *s)
{
	...
	{
		jso->o.c_string.str.ptr = _strdup(s);
...
static int json_object_binary_to_json_string(struct json_object *jso,
											 struct printbuf *pb,
											 int level,
											 int flags)
{
	...
	// hexdump
	if (jso->o.c_binary.len > 0)
	{
		char sbuf[3];
		for (size_t i = 0; (i < jso->o.c_binary.len) && (i < 8); i++)
		{
			snprintf(sbuf, 3, "%2x", *(jso->o.c_binary.ptr + i));
			printbuf_memappend(pb, sbuf, 2);
		}
	}
...
struct json_object *json_object_new_binary(unsigned char *s, int len)
{
	...
	if (json_object_set_binary(jso, s, len) != 1)
	{
		json_object_generic_delete(jso);
		errno = ENOMEM;
		return NULL;
...
unsigned char *json_object_get_binary(struct json_object *jso, int *len)
{
	if (!jso)
		return NULL;
	switch (jso->o_type)
	{
	case json_type_binary:
		*len = jso->o.c_binary.len;
		return jso->o.c_binary.ptr;
	default:
		return NULL;
	}
}

int json_object_set_binary(json_object *jso, unsigned char *s, int len)
{
	char *dstbuf;
	if (jso == NULL || jso->o_type != json_type_binary)
		return 0;

	dstbuf = (char *)malloc(len);
	if (dstbuf == NULL)
		return 0;
	if (jso->o.c_binary.ptr != NULL)
		free(jso->o.c_binary.ptr);
	jso->o.c_binary.ptr = dstbuf;
	jso->o.c_binary.len = len;
	memcpy(dstbuf, (const void *)s, len);
	return 1;
}
